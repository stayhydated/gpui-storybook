...
mod theme_import {
    ...
    pub(super) const INVALID_JSON: &str = "Invalid theme JSON.";

    #[validator]
    #[derive(Clone, Debug)]
    pub struct ThemeImportValidation {
        #[koruma(skip_capture)]
        #[allow(dead_code)]
        input: Option<String>,
    }

    impl Validate<String> for ThemeImportValidation {
        fn validate(&self, value: &String) -> bool {
            serde_json::from_str::<ThemeColor>(value).is_ok()
        }
    }

    impl fmt::Display for ThemeImportValidation {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            f.write_str(INVALID_JSON)
        }
    }

    #[derive(Koruma, KorumaAllDisplay)]
    struct ThemeImport {
        #[koruma(ThemeImportValidation)]
        json: String,
    }

    pub(super) fn validate(json: &str) -> Result<(), String> {
        let import = ThemeImport {
            json: json.to_owned(),
        };
        import.validate().map_err(|errors| {
            errors
                .json()
                .all()
                .next()
                .map(|error| error.to_string())
                .unwrap_or_else(|| INVALID_JSON.to_owned())
        })
    }
}
...
impl ThemeDraft {
    ...
    pub fn import_json(&mut self, json: &str, cx: &mut App) -> Result<(), ThemeDraftError> {
        theme_import::validate(json).map_err(ThemeDraftError)?;
        let imported: ThemeColor = serde_json::from_str(json)
            .map_err(|_| ThemeDraftError(theme_import::INVALID_JSON.to_owned()))?;
