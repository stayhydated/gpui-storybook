...
#[derive(Action, Clone, Copy, Eq, PartialEq)]
#[action(namespace = storybook_workbench, no_json)]
struct SelectBackground {
    background: StoryCanvasBackground,
}
...
impl StoryWorkbench {
    ...
    fn select_background(
        ...
        self.state.update(cx, |state, cx| {
            state.set_background(action.background, cx);
        });
    }
                                    ...
                                    h_flex()
                                        ...
                                        .child(
                                            Button::new(format!("theme-reset-{name}"))
                                                ...
                                                .on_click(cx.listener(move |this, _, _, cx| {
                                                    let result =
                                                        this.theme_draft.reset_color(&name, cx);
...
impl Render for StoryWorkbench {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let state_for_grid = self.state.clone();
        ...
        v_flex()
            ...
            .on_action(cx.listener(Self::select_background))
            .size_full()
            .overflow_hidden()
            .child(
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(
                                Button::new("workbench-background")
                                    .label(presentation.background.label())
                                    .xsmall()
                                    .dropdown_menu(move |menu, _, _| {
                                        StoryCanvasBackground::ALL.into_iter().fold(
                                            menu,
                                            |menu, background| {
                                                menu.menu_with_check(
                                                    background.label(),
                                                    background == presentation.background,
                                                    Box::new(SelectBackground { background }),
                                                )
                                            },
                                        )
                                    }),
                            ...
                            .child(
                                Checkbox::new("workbench-grid")
                                    .label("Grid")
                                    .checked(presentation.grid)
                                    .on_click(move |checked, _, cx| {
                                        state_for_grid.update(cx, |state, cx| {
                                            state.set_grid(*checked, cx);
                                        });
                                    }),
                            ),
                    ),
            ...
            .child(
                div()
                    ...
                    .child(match self.selected_tab {
                        WorkbenchTab::Controls => self.render_controls(window, cx),
                        WorkbenchTab::Theme => self.render_theme(cx),
                        WorkbenchTab::Inspect => self.render_inspect(cx),
                    }),
            )
    }
}
