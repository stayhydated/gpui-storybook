...
2. App startup calls `gpui_storybook::init`, wiring language, locale, and core runtime setup.
3. `generate_stories` reads inventory entries, orders them, and returns story containers.
4. `Gallery::view` (core crate) renders the sidebar and active story content.
